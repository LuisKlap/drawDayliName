...
export function Home() {
  ...
  function handleCreateNewCycle() {
    if (nameList.length === 0) {
      // Verifique se a lista de nomes está vazia.
    ...
    setNameList((prevNameList) => {
      // Remova o nome sorteado da lista.
  ...
  return (
    <HomeContainer>
      <form onSubmit={handleSubmit(handleCreateNewCycle)} action="">
        ...
        <label htmlFor="task">Sorteado da vez: {activeCycle?.task}</label>
        <CountDownContainer>
          <span>{minutes[0]}</span>
          <span>{minutes[1]}</span>
        ...
        <AlignButtons>
          <StartCountDownButton disabled={isPaused} type="submit">
            <Play size={24} />
            Começar
          </StartCountDownButton>
